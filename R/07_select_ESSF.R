...
samp_years_vec <- vect(samp_years, geom = c("longitude","latitude"), crs="+proj=longlat +datum=WGS84")


# read in ClimateNA variables ----------------------------------------------------------------------
varList <- c("mat","cmd")

# run raster download put in temp folder, only 800m or 4km available options
ClimateNAr::rasterDownload(region='BC', res='800m', period='Normal_1981_2010',
                           varList=varList,
                           sDir='D:/Github/sortie_cc_biotic/01_climate_sensitive_growth/01_data')

# stack and clip downloaded rasters
t <- "D:/Github/sortie_cc_biotic/01_climate_sensitive_growth/01_data/BC/800m/Normal_1981_2010"
stk <- ClimateNAr::rasterStack(t, varList=varList, rType='tif')


# intersect PSPs and ClimateNA ------------------------

clim_psp <- extract(stk, samp_years_vec)

#add climate data to PSP data
samp_years <- cbind(samp_years, clim_psp)



# Remove PSPs that don't meet other criteria ------------------------------------------------------
